...
from Bestie_Robot import dispatcher, REDIS
...
def makepack_internal(
    ...
    try:
        ...
        if png_sticker:
            sticker_pack_name = (
                f"{name}'s stic-pack by @{context.bot.username}" + extra_version
            )
        ...
        if tgs_sticker:
            sticker_pack_name = (
                f"{name}'s ani-pack by @{context.bot.username}" + extra_version
            )
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
...
KANG_HANDLER = DisableAbleCommandHandler(
    ["kang", "steal"], addsticker, pass_args=True, run_async=True
)
...
ADD_FSTICKER_HANDLER = DisableAbleCommandHandler(
    ["addfav", "afv"], add_fvrtsticker, pass_args=True, run_async=True
)
REMOVE_FSTICKER_HANDLER = DisableAbleCommandHandler(
    ["removefslav", "rfv"], remove_fvrtsticker, pass_args=True, run_async=True
)
MY_FSTICKERS_HANDLER = DisableAbleCommandHandler(
    ["myfav", "mfv"], list_fvrtsticker, run_async=True
)
