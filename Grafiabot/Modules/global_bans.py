...
import Bestie_Robot.modules.sql.global_bans_sql as sql
from Bestie_Robot.modules.sql.users_sql import get_user_com_chats
...
from Bestie_Robot.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID:
        message.reply_text(
            "This Hunter is my Owner, I really don't want to get deleted."
        )
        return
    ...
    if int(user_id) in WOLVES:
        message.reply_text(
            "That's a D-Rank Hunter! They are immune to all type of bans."
        )
        return
